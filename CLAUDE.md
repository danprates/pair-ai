...
A Claude Code plugin called `pair` that ships three user-invocable skills:
`/pair:commit`, `/pair:code-review`, `/pair:pull-request`. There is no
...
- `plugins/pair/skills/code-review/template.md` and `plugins/pair/skills/pull-request/template.md` — the built-in output templates referenced inside the skills via `${CLAUDE_PLUGIN_ROOT}`.
...
- **Output paths are part of the public contract.** `commit` writes `./tmp/diff.txt`; `code-review` writes `./tmp/diff.txt` and `./tmp/code-review.md`; `pull-request` writes `./tmp/diff.txt` and `./tmp/pull-request.md`. Users rely on these paths — do not rename them.
...
- All three skills accept a trailing **language argument** (`en` default, e.g. `pt-BR`, `es`); the output (commit message / review body / PR description) must be written in that language while the skill's own status report can follow suit.
