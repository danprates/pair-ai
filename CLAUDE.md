...
A Claude Code plugin called `pair` that ships nine user-invocable skills:
`/pair:commit`, `/pair:code-review`, `/pair:pull-request`, `/pair:handoff`, `/pair:explain`, `/pair:guide`, `/pair:verify`, `/pair:impact-analysis`, `/pair:create-task`. There is no
...
- [`plugins/pair/skills/<skill>/SKILL.md`](plugins/pair/skills/) — each skill is a markdown file with YAML frontmatter. Frontmatter fields that matter:
  ...
  - `context: fork` — runs the skill in an isolated subagent with a fresh context window. Only `commit` carries this field; the remaining skills run in the main conversation context.
- `plugins/pair/skills/code-review/template.md`, `plugins/pair/skills/pull-request/template.md`, `plugins/pair/skills/handoff/template.md`, `plugins/pair/skills/explain/template.md`, `plugins/pair/skills/guide/template.md`, `plugins/pair/skills/verify/template.md`, `plugins/pair/skills/impact-analysis/template.md`, `plugins/pair/skills/create-task/template.md`, `plugins/pair/skills/create-task/template-orchestrator.md`, and `plugins/pair/skills/create-task/template-validate.md` — the built-in output templates referenced inside the skills via `${CLAUDE_PLUGIN_ROOT}`. The `create-task` skill has three variants: `template.md` (implementation tasks), `template-orchestrator.md` (000 orchestrator), and `template-validate.md` (999 validation task).
...
- `tmp/` is gitignored; it holds the user-project output of the skills (`diff.txt`, `code-review.md`, `pull-request.md`, `handoff.md`, `explain.md`, `guide.md`, `verify.md`, `impact-analysis.md`) and task suites (`tasks/<suite-id>/000-orchestrator.md`, `001–NNN` implementation tasks, `999-validate.md`); it is also the location users use to override templates (`tmp/templates/<skill>.md` and `tmp/templates/create-task-<variant>.md`).
...
- **Output paths are part of the public contract.** `commit` writes `./tmp/diff.txt`; `code-review` writes `./tmp/diff.txt` and `./tmp/code-review.md`; `pull-request` writes `./tmp/diff.txt` and `./tmp/pull-request.md`; `handoff` writes `./tmp/diff.txt` and `./tmp/handoff.md`; `explain` writes `./tmp/diff.txt` and `./tmp/explain.md`; `guide` writes `./tmp/guide.md` only (no diff — it is prospective); `verify` writes `./tmp/diff.txt` and `./tmp/verify.md`; `impact-analysis` writes `./tmp/diff.txt` and `./tmp/impact-analysis.md`; `create-task` writes a suite of files into `./tmp/tasks/<suite-id>/` (no diff — prospective): `000-orchestrator.md`, one or more `NNN-<slug>.md` implementation tasks (numbered 001–998), and `999-validate.md`. Suite IDs use the Jira/Azure ticket ID as a prefix if detected (e.g. `JIRA-1234-add-auth`), otherwise a plain slug. In ADD mode (`$1` is an existing suite folder), it appends new implementation tasks and regenerates `000-orchestrator.md` and `999-validate.md`. Users rely on these paths — do not rename them.
- **Template resolution order is fixed** and must be preserved when editing either skill:
  ...
  `create-task` has **three template variants** (each follows the same two-step override → plugin-default order): `create-task.md` (implementation tasks), `create-task-orchestrator.md` (the 000 orchestrator), and `create-task-validate.md` (the 999 validation task). User overrides live at `./tmp/templates/create-task-<variant>.md`; plugin defaults at `${CLAUDE_PLUGIN_ROOT}/skills/create-task/template-<variant>.md` (except the implementation template, which uses the plain `template.md` name).
- **Global issue numbering** in `code-review` and `verify` is load-bearing: issues are numbered sequentially across all categories (never reset per category) so users can say "fix 2 / ignore 3". Any template override — including user overrides in `tmp/templates/code-review.md` or `tmp/templates/verify.md` — must preserve this. The default templates restate this rule in their "Format rules" sections; keep those restatements in sync with the skills.
...
- The skill itself should end by reporting back which template was used and the output path(s), matching the pattern of the existing skills.
