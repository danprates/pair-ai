...
A Claude Code plugin called `pair` that ships five user-invocable skills:
`/pair:commit`, `/pair:code-review`, `/pair:pull-request`, `/pair:handoff`, `/pair:explain`. There is no
...
- `plugins/pair/skills/code-review/template.md`, `plugins/pair/skills/pull-request/template.md`, `plugins/pair/skills/handoff/template.md`, and `plugins/pair/skills/explain/template.md` — the built-in output templates referenced inside the skills via `${CLAUDE_PLUGIN_ROOT}`.
...
- `tmp/` is gitignored; it holds the user-project output of the skills (`diff.txt`, `code-review.md`, `pull-request.md`) and is also the location users use to override templates (`tmp/templates/code-review.md`, `tmp/templates/pull-request.md`).
...
- **Output paths are part of the public contract.** `commit` writes `./tmp/diff.txt`; `code-review` writes `./tmp/diff.txt` and `./tmp/code-review.md`; `pull-request` writes `./tmp/diff.txt` and `./tmp/pull-request.md`; `handoff` writes `./tmp/diff.txt` and `./tmp/handoff.md`; `explain` writes `./tmp/diff.txt` and `./tmp/explain.md`. Users rely on these paths — do not rename them.
...
- All skills accept a trailing **language argument** (`en` default, e.g. `pt-BR`, `es`); the output (commit message / review body / PR description / handoff / explain document) must be written in that language while the skill's own status report can follow suit.
